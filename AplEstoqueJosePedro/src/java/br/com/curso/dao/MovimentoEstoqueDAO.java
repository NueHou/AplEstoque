...
public class MovimentoEstoqueDAO implements GenericDAO{
    ...
    public Boolean inserir(Object objeto) {
        ...
        String sql = "insert into MovimentoEstoque (entradaSaida, documento, data, quantidade, valorMovimento, id_Produto, idTipoMovimento, idFuncionario)"
                + "values(?,?,?,?,?,?,?,?)";
        try{
            ...
            stmt.setInt(6, oMovimentoEstoque.getId_Produto().getId_Produto());
            stmt.setInt(7, oMovimentoEstoque.getIdTipoMovimento().getIdTipoMovimento());
            stmt.setInt(8, oMovimentoEstoque.getIdFuncionario().getIdFuncionario());
            stmt.execute();
            conexao.commit();
            return true;
        }catch(Exception ex){
    ...
    MovimentoEstoque oMovimentoEstoque = (MovimentoEstoque) objeto;
        PreparedStatement stmt = null;
        String sql = "update MovimentoEstoque set entradaSaida=?,documento=?, data=?, quantidade=?, valorMovimento=?, id_Produto=?, idTipoMovimento=?, idFuncionario=?  "
                + "where idMovimento=?";
        try {
            stmt = conexao.prepareStatement(sql);
            stmt.setString(1, oMovimentoEstoque.getEntradaSaida());
    ...
    int idMovimento = numero;
        ...
        try {
            ...
            while (rs.next()) {
                ...
                oMovimentoEstoque.setId_Produto((Produto) oProdutoDAO.carregar(rs.getInt("id_Produto")));
                
                TipoMovimentoDAO oTipoMovimentoDAO = new TipoMovimentoDAO();
                oMovimentoEstoque.setIdTipoMovimento((TipoMovimento) oTipoMovimentoDAO.carregar(rs.getInt("idTipoMovimento")));
                
                FuncionarioDAO oFuncionarioDAO = new FuncionarioDAO();
                oMovimentoEstoque.setIdFuncionario((Funcionario) oFuncionarioDAO.carregar(rs.getInt("idFuncionario")));
            }
